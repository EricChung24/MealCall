...
export async function saveMeal(meal:Meal,householdId:string){const supabase=createClient();if(!supabase)throw new Error('Supabase 未設定');const {data:row,error}=await supabase.from('meals').upsert({household_id:householdId,date:meal.date,slot:meal.slot,title:meal.title,cook_id:null,status:meal.status},{onConflict:'household_id,date,slot'}).select('id').single();if(error)throw error;await supabase.from('meal_ingredients').delete().eq('meal_id',row.id);const {error:itemError}=await supabase.from('meal_ingredients').insert(meal.ingredients.map(x=>({meal_id:row.id,ingredient_id:x.ingredientId,planned_amount:x.plannedAmount,reserved_amount:x.reservedAmount,consumed_amount:x.consumedAmount})));if(itemError)throw itemError;return row.id;}
